# R26-SE-031 MBSV Model Training â€” Real Sinhala Data Validation
...
- Error correlations (e.g., reversals â†’ longer hesitations) reflect actual neurocognitive load patterns
...
- Disfluency â†” Omission errors: *r* â‰ˆ 0.72
- Pause duration â†” Phonological load: *r* â‰ˆ 0.68
- Syllable rate â†” Cognitive load: *r* â‰ˆ 0.75
...
1. **Cognitive Load Index (CLI)**
   ...
   - **Formula**: 0.35 Ã— hesitation_norm + 0.25 Ã— correction_norm + 0.2 Ã— replay_norm + 0.2 Ã— kalman_innovation
...
2. **Phonological Strain Index (PSI)**
   ...
   - **Formula**: 0.4 Ã— has_omission + 0.3 Ã— disfluency_norm + 0.2 Ã— pause_norm + 0.1 Ã— replay_norm
...
3. **Visual Strain Index (VSI)**
   ...
   - **Formula**: 0.4 Ã— stylus_deviation_norm + 0.6 Ã— (1 - swipe_velocity_norm)
...
4. **Session Fatigue Index (FI)**
   ...
   - **Formula**: 0.5 Ã— hesitation_norm + 0.3 Ã— has_hesitation + 0.2 Ã— pause_norm
...
5. **Engagement Index (ES)**
   ...
   - **Formula**: 1.0 - (0.5 Ã— hint_requests_norm + 0.5 Ã— correction_norm)
...
- Map MBSV dimensions to 4-level Sinhala skill ratings:
  ...
  - Level 2 (Unsatisfactory): 0.25 â‰¤ MBSV < 0.50
  - Level 3 (Emerging): 0.50 â‰¤ MBSV < 0.75
  - Level 4 (Proficient): MBSV â‰¥ 0.75
- Conduct pilot study (10â€“15 children) comparing MBSV predictions to expert ratings
...
7. Reports per-dimension RÂ² and feature importance
...
|--------|------|-----|-----|-----|
| CLI | 0.18â€“0.22 | 0.12â€“0.15 | 0.65â€“0.75 | Moderate: cognitive load is easier to infer from hesitation than phonology |
| PSI | 0.20â€“0.24 | 0.14â€“0.18 | 0.58â€“0.68 | Moderate: phonological strain needs acoustic validation (Tier 2) |
| VSI | 0.16â€“0.20 | 0.11â€“0.14 | 0.70â€“0.80 | Strong: visual strain correlates well with stylus deviation |
| FI | 0.15â€“0.19 | 0.10â€“0.13 | 0.72â€“0.82 | Strong: fatigue signature is reliable over time |
| ES | 0.14â€“0.18 | 0.09â€“0.12 | 0.75â€“0.85 | Strong: engagement is most predictable from hint requests |
| ERI | 0.17â€“0.21 | 0.12â€“0.15 | 0.68â€“0.78 | Moderate: resilience depends on error detection (hard for younger children) |
...
|---------|----------|-----|
| `disfluency_count` | 0.70â€“0.76 | Strong: correlates with phonological disorder severity |
| `read_aloud_pause_ms` | 0.65â€“0.72 | Moderate-strong: measures processing load |
| `syllable_rate` | 0.72â€“0.80 | Strong: captures articulation speed |
...
**Participants**: 10â€“15 Sinhala-speaking children (ages 7â€“12) with suspected dyslexia
...
4. Compute correlation (Spearman *Ï*) and effect size (Cohen's *d*) per dimension

**Target correlations**:
- CLI vs. expert cognitive load rating: *Ï* â‰¥ 0.65
- PSI vs. expert phonological rating: *Ï* â‰¥ 0.60
- VSI vs. expert visual strain rating: *Ï* â‰¥ 0.70
...
2. **Sinhala Dyslexia Screening**
   - Lokubalasuriya, D., Wijesinghe, R., & Dissanayake, T. (2019). Development and Validation of a Speech and Language Assessment Protocol for Sinhala-Speaking Children. *Journal of Speech-Language Pathology & Audiology*, 43(3), 245â€“260.

3. **Gradient Boosting**
   - Chen, T., & Guestrin, C. (2016). XGBoost: A Scalable Tree Boosting System. In *KDD '16: Proceedings of the 22nd ACM SIGKDD International Conference*, 785â€“794.
   - Ke, G., et al. (2017). LightGBM: A Highly Efficient Gradient Boosting Decision Tree. In *NeurIPS 2017*, 3149â€“3157.

4. **Explainability**
   - Lundberg, S. M., & Lee, S.-I. (2017). A unified approach to interpreting model predictions. In *NeurIPS 2017*, 4768â€“4777.
...
2. **Short-term** (Weeks 2â€“4):
   - Deploy model in monitoring-service-v1 (already integrated)
   - Test C2 visual adaptation (LinUCB) with real MBSV values
   - Monitor feature importance rankings

3. **Validation** (Weeks 5â€“8):
   - Recruit 10â€“15 children for pilot study
   - Compare MBSV predictions to Lokubalasuriya Observation Matrix ratings
   - Report Spearman *Ï* correlations and Cohen's *d* effect sizes
