"""
tests/test_integration.py -- R26-SE-031  (v3.0)
==============================================
Full integration test suite covering:
  - All 11 module smoke tests (from tests/test_smoke.py)
  - Trained model loading and inference (LightGBM, RF, LinUCB)
  - End-to-end pipeline: TelemetryPayload â†’ MBSV â†’ downstream services
...
# â”€â”€ Register shared package â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async def main():
    ...
    try:
        ...
        _ok("shared/schemas.py â€” all v3.0 types present, BLENDING_GAME in ActivityType")
    ...
    try:
        ...
        _ok(f"bkt_engine.py  p_know {round(bf,3)}â†’{round(af,3)}  threshold=0.833")
    except Exception as e:
    ...
    try:
        ...
        assert lgbm_path.exists(), "Model not found â€” run scripts/run_all_training.py first"
    ...
    try:
        ...
        assert rf_path.exists(), "Model not found â€” run scripts/run_all_training.py first"
    ...
    try:
        ...
        assert linucb_path.exists(), "Model not found â€” run scripts/run_all_training.py first"
    ...
    # E2E: TelemetryPayload â†’ MBSV â†’ InterventionCheckPayload
    try:
        from shared.schemas import (
        ...
        _ok(f"E2E Pipeline: Telemetryâ†’MBSVâ†’BKTâ†’Intervention  stage={result['stage']}  activity={result.get('activity_type','N/A')}")
    except Exception as e:
...
if __name__ == "__main__":
    ...
    if FAIL == 0:
        ...
        print("  SOME TESTS FAILED â€” check output above")
        sys.exit(1)
