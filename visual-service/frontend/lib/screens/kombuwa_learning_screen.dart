...
import 'letter_bubble_game_kombuwa.dart';

class KombuwaLearningScreen extends StatefulWidget {
  const KombuwaLearningScreen({super.key});

  @override
  State<KombuwaLearningScreen> createState() => _KombuwaLearningScreenState();
}

class _KombuwaLearningScreenState extends State<KombuwaLearningScreen> {
  ...
  void _nextPage() {
    if (_currentPage < 7) {
      ...
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(builder: (_) => const LetterBubbleGameKombuwa()),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('කොම්බුව ඉගෙන ගමු', style: TextStyle(color: Colors.black87, fontWeight: FontWeight.bold)),
        ...
        children: [
          TweenAnimationBuilder<double>(
            ...
            child: const Padding(
              ...
              child: Text(
                'කොම්බුව අකුරකට එකතු කළ විට ශබ්දය වෙනස් වේ.',
                textAlign: TextAlign.center,
...
class _InteractiveTracingPageState extends State<InteractiveTracingPage> {
  ...
  Path _getKombuwaPath(Size size) {
    Path path = Path();
    double cx = size.width / 2;
    double cy = size.height / 2 - 20;

    // 1. FIRST DRAW THE CIRCLE IN THE MIDDLE
    path.moveTo(cx, cy);
    // Right and Down
    path.arcToPoint(Offset(cx + 25, cy + 25), radius: const Radius.circular(20), clockwise: true);
    // Left and Up (Completes the circle)
    path.arcToPoint(Offset(cx - 5, cy - 15), radius: const Radius.circular(25), clockwise: true);
    
    // 2. AND THEN RIGHT, DOWN, LEFT, UP
    // RIGHT
    path.arcToPoint(Offset(cx + 55, cy + 15), radius: const Radius.circular(35), clockwise: true);
    // DOWN
    path.arcToPoint(Offset(cx + 15, cy + 95), radius: const Radius.circular(55), clockwise: true);
    // LEFT
    path.arcToPoint(Offset(cx - 65, cy + 35), radius: const Radius.circular(65), clockwise: true);
    // UP (Extending the tail longer)
    path.arcToPoint(Offset(cx - 10, cy - 75), radius: const Radius.circular(75), clockwise: true);
  ...
  void _onPanEnd(DragEndDetails details) {
    ...
    RenderBox renderBox = context.findRenderObject() as RenderBox;
    Path guidePath = _getKombuwaPath(renderBox.size);
        ...
        children: [
          Align(
            ...
            child: Padding(
              ...
              child: Text(
                _isCompleted ? 'නියමයි!' : 'කොම්බුව අඳිමු!',
...
class _TracingPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (showGuide) {
      ...
      // Start dot - dynamically positioned at the first point of the path!
      if (guidePath.computeMetrics().isNotEmpty) {
        PathMetric firstMetric = guidePath.computeMetrics().first;
        Offset startPos = firstMetric.getTangentForOffset(0)?.position ?? Offset(centerX, centerY);
        Paint startDotPaint = Paint()..color = Colors.redAccent;
        canvas.drawCircle(startPos, 18, startDotPaint);
      }
    }
...
class KombuwaPainter extends CustomPainter {
  final Color color;
  KombuwaPainter({required this.color});
  ...
  @override
  bool shouldRepaint(covariant KombuwaPainter oldDelegate) => oldDelegate.color != color;
}
        ...
        Align(
          ...
          child: Padding(
            ...
            child: AnimatedSwitcher(
              duration: const Duration(milliseconds: 300),
              child: Text(
                step == 0 ? 'අකුර ඔබන්න!' : (step == 1 ? 'කොම්බුව ඔබන්න!' : 'නියමයි!'),
                key: ValueKey(step),
                style: const TextStyle(fontSize: 32, fontWeight: FontWeight.bold, color: Colors.deepPurple),
              ),
            ),
          ...
          TweenAnimationBuilder<double>(
            ...
            builder: (context, val, child) {
              ...
              return AnimatedAlign(
                duration: const Duration(milliseconds: 500),
                curve: Curves.easeInBack,
                alignment: Alignment(step == 2 ? -0.5 : alignX, 0),
                child: child,
              );
            },
            child: GestureDetector(
              ...
              child: AnimatedContainer(
                ...
                child: SizedBox(
                  ...
                  height: 130,
                  child: CustomPaint(
                    painter: KombuwaPainter(color: Colors.redAccent),
                  ),
                ),
              ),
            ),
