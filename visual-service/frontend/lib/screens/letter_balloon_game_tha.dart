...
import 'word_start_letter_game_tha.dart';
...
class LetterBalloonGameTha extends StatefulWidget {
  const LetterBalloonGameTha({super.key});

  @override
  State<LetterBalloonGameTha> createState() => _LetterBalloonGameThaState();
}

class _LetterBalloonGameThaState extends State<LetterBalloonGameTha> with SingleTickerProviderStateMixin {
  final String studentId = "student_001";
  final String targetLetter = "ත";
  final List<String> distractorLetters = ["ප", "ම", "ග", "ර"];
  ...
  List<BackgroundBalloon> _bgBalloons = [];
    ...
    try {
      await http.post(
        ...
        body: jsonEncode({
          ...
          "task_id": "letter_balloon_game_pa_01",
  ...
  Future<void> _updateMastery(bool correct) async {
    ...
    try {
      await http.post(
        ...
        body: jsonEncode({
          "student_id": studentId,
          "skill_id": "vowel_recognition_balloons",
          "is_correct": correct,
  ...
  void _initPositions(Size size) {
    ...
    // Spawn initial background decorative balloons
    for (int i = 0; i < 8; i++) {
      _bgBalloons.add(BackgroundBalloon(
    ...
    // Stagger spawn the initial game balloons directly as normal
    for (int i = 0; i < 3; i++) {
  ...
  void _spawnGrowingBalloon() {
    ...
    // Spawn from balloon gun nozzle (character is at bottom middle of screen)
  ...
  void _updateGame() {
    ...
    setState(() {
      // 1. Update background decorative balloons
      for (var bg in _bgBalloons) {
        bg.y -= bg.speed;
        bg.wavePhase += 0.01;
      ...
      // 2. Update gameplay balloons
      List<Balloon> deadBalloons = [];
      
      // Update Owl Blow Progress
      if (_isOwlBlowing) {
        _owlBlowingProgress += 0.012; // slow down to make animation clear
        
        // At 50% blow phase, spawn growing balloon
        if (_owlBlowingProgress >= 0.50 && _owlBlowingProgress < 0.85 && _growingBalloon == null) {
          _spawnGrowingBalloon();
        }

        // Scale growing balloon
        if (_growingBalloon != null && _growingBalloon!.state == 'growing') {
          // Normalize blow progress between 0.50 and 0.85
          double norm = ((_owlBlowingProgress - 0.50) / 0.35).clamp(0.0, 1.0);
          _growingBalloon!.radius = _growingBalloon!.targetRadius * norm;
          
          // keep it locked at the gun nozzle
          final double charX = _screenSize.width / 2;
          final double charY = _screenSize.height - 80;
          _growingBalloon!.x = charX + 25;
          _growingBalloon!.y = charY - 15;
        }

        // At 85% release phase, release balloon to float
        if (_owlBlowingProgress >= 0.85 && _growingBalloon != null && _growingBalloon!.state == 'growing') {
          _growingBalloon!.state = 'normal';
          _growingBalloon = null;
        }
      ...
      for (var db in deadBalloons) {
        _balloons.remove(db);
        if (!isCompleted) {
          _startBlowingBalloon();
        }
      }
      ...
      // Keep balloon count healthy (at least 3 balloons)
      int activeCount = _balloons.where((b) => b.state == 'normal' || b.state == 'growing').length;
  ...
  void _handleBalloonTap(Balloon balloon) {
    if (isCompleted || balloon.state == 'correct' || balloon.state == 'growing') return;
    ...
    if (balloon.letter == targetLetter) {
      ...
      _spawnParticles(balloon.x, balloon.y, const Color(0xFF58CC02));
      _updateMastery(true);
      ...
      setState(() {
        balloon.state = 'incorrect';
        balloon.shakePhase = 0.0;
        errorsCount++;
      });
      
      _spawnParticles(balloon.x, balloon.y, const Color(0xFFFF9600));
  ...
  void _launchFireworks() {
    ...
    Future.delayed(const Duration(seconds: 4), () {
      ...
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(builder: (_) => const WordStartLetterGameTha()),
      );
    });
  }
          ...
          child: AlertDialog(
            ...
            content: Column(
              ...
              children: [
                ...
                const SizedBox(height: 10),
                const Text(
                  "ඔබ සාර්ථකව 'ත' අකුර හඳුනා ගත්තා!", // You successfully identified the letter 'ත'!
      ...
      body: SafeArea(
        child: Stack(
          ...
          children: [
            ...
            // 2. Render decorative background balloons
            ..._bgBalloons.map((bg) => Positioned(
                ...
                children: [
                  Positioned(
                    ...
                    child: CustomPaint(
                      ...
                      painter: BalloonKnotPainter(color: Colors.white.withOpacity(0.22)),
                    ),
                  ),
                  Container(
                    width: bg.radius * 1.8,
                    height: bg.radius * 2.2,
                    decoration: BoxDecoration(
                      borderRadius: BorderRadius.all(Radius.elliptical(bg.radius * 0.9, bg.radius * 1.1)),
            ...
            // 4. Render gameplay balloons containing letters
            ..._balloons.map((b) {
              ...
              Color balloonBg = Colors.white.withOpacity(0.30);
              Color borderCol = Colors.black.withOpacity(0.8);
              Color textCol = Colors.black87;

              if (b.state == 'correct') {
                balloonBg = const Color(0xFF58CC02).withOpacity(b.opacity);
                borderCol = const Color(0xFF78C800).withOpacity(b.opacity);
                textCol = Colors.white.withOpacity(b.opacity);
              } else if (b.state == 'incorrect') {
                balloonBg = const Color(0xFFFF9600);
                ...
                child: GestureDetector(
                  ...
                  child: Transform.scale(
                    ...
                    child: Opacity(
                      ...
                      child: SizedBox(
                        width: b.radius * 2,
                        height: b.radius * 2.4, // balloons are taller
                        child: Stack(
                          alignment: Alignment.center,
                          clipBehavior: Clip.none,
                          children: [
                            ...
                            Positioned(
                              ...
                              child: CustomPaint(
                                ...
                                painter: BalloonKnotPainter(color: balloonBg),
                              ),
                            ),
                            // Balloon Body
                            Container(
                              ...
                              height: b.radius * 2.3,
                              decoration: BoxDecoration(
                                borderRadius: BorderRadius.all(Radius.elliptical(b.radius * 0.9, b.radius * 1.15)),
                                color: balloonBg,
                                boxShadow: [
                                  BoxShadow(
...
class DuoOwlPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Balloon Gun
    _drawBalloonGun(canvas, handPos, gunAngle, fillPaint, strokePaint);
  ...
  void _drawBalloonGun(Canvas canvas, Offset pos, double angle, Paint fillPaint, Paint strokePaint) {
    canvas.save();
    ...
    // Soft balloon soapy layer reflection (shiny blue inside loop nozzle)
...
class BalloonKnotPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    stringPath.moveTo(size.width / 2, size.height);
    stringPath.quadraticBezierTo(size.width * 0.2, size.height + 10, size.width / 2, size.height + 20);
    stringPath.quadraticBezierTo(size.width * 0.8, size.height + 30, size.width / 2, size.height + 40);
