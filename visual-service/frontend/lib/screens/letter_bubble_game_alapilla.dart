...
import 'word_start_letter_game_alapilla.dart';
...
class LetterBubbleGameAlapilla extends StatefulWidget {
  const LetterBubbleGameAlapilla({super.key});

  @override
  State<LetterBubbleGameAlapilla> createState() => _LetterBubbleGameAlapillaState();
}

class _LetterBubbleGameAlapillaState extends State<LetterBubbleGameAlapilla> with SingleTickerProviderStateMixin {
  final String studentId = "student_001";
  final List<String> targetLetters = ["කා", "මා", "ගා", "සා", "ලා", "බා", "තා", "දා"];
  final List<String> distractorLetters = ["ක", "ම", "ග", "ස", "ල", "බ", "ත", "ද"];
    ...
    try {
      await http.post(
        ...
        body: jsonEncode({
          ...
          "task_id": "letter_bubble_game_alapilla_01",
  ...
  Future<void> _updateMastery(bool correct) async {
    ...
    try {
      await http.post(
        ...
        body: jsonEncode({
          "student_id": studentId,
          "skill_id": "alapilla_recognition_bubbles",
          "is_correct": correct,
  ...
  void _initPositions(Size size) {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      String letter = _random.nextDouble() < 0.35 
          ? targetLetters[_random.nextInt(targetLetters.length)] 
  ...
  void _handleBubbleTap(Bubble bubble) {
    ...
    if (targetLetters.contains(bubble.letter)) {
      // CORRECT CELEBRATION
  ...
  void _launchFireworks() {
    ...
    Future.delayed(const Duration(seconds: 4), () {
      ...
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(builder: (_) => const WordStartLetterGameAlapilla()),
      );
    });
  }
          ...
          child: AlertDialog(
            ...
            content: Column(
              ...
              children: [
                ...
                const SizedBox(height: 10),
                const Text(
                  "ඔබ සාර්ථකව ඇලපිල්ල සහිත අකුරු හඳුනා ගත්තා!", // You successfully identified the letters with Alapilla!
    ...
    setState(() {
      ...
      for (int i = 0; i < 3; i++) {
        ...
        _bubbles.add(Bubble(
          ...
          x: startX,
          y: startY,
          letter: (i == 0) ? targetLetters[_random.nextInt(targetLetters.length)] : distractorLetters[i % distractorLetters.length],
          radius: targetR,
          targetRadius: targetR,
          speed: 0.6 + _random.nextDouble() * 0.4,
          wavePhase: _random.nextDouble() * pi * 2,
